...
class AuthManager {
    ...
    // ─────────────────────────────────────────────
    //  FORGOT PASSWORD — local-only reset (no email server needed)
    // ─────────────────────────────────────────────
    ...
    async resetPassword(email, newPassword) {
        ...
        if (users[idx].provider) return { ok: false, error: 'This account uses ' + users[idx].provider + ' sign-in. Password reset is not available.' };
...
class SharedLibrary {
    ...
    publish(chapter, user) {
        ...
        const entry = {
            ...
            // Multi-page sections for cheat-sheet themed rendering in the library.
            // Each element: { title: string, content: string, tags: string[] }
            // Set by publishToLibrary() when multiple pages are in the stream.
            sections: chapter.sections || null,
        };
...
function libRenderCards() {
    ...
    const entries = window.LIBRARY.search(query, category);
    ...
    if (entries.length === 0) {
        ...
        const hasAny = window.LIBRARY.getAll().length > 0;
    ...
    const libUser = window.AUTH ? window.AUTH.getCurrentUser() : null;
...
function libBuildCard(entry, libUser) {
    const isOwner = libUser && entry.authorId === libUser.userId;
...
function libOpenModal(id) {
    const entry = window.LIBRARY.getById(id);
    if (!entry) return;
...
// ── Actions ───────────────────────────────────────────────────────────────────
function libCloneEntry(id) {
    const clone = window.LIBRARY.buildClone(id);
    if (!clone) { libToast('Note not found'); return; }

    // Store pending clone — main app picks it up on next init (or immediately
    // since we're already on index.html)
    const pendingKey = window.AUTH
        ? window.AUTH.getStorageKey('nb_pending_clone')
        : 'nb_pending_clone';
...
async function libDeleteEntry(id) {
    const libUser = window.AUTH ? window.AUTH.getCurrentUser() : null;
    if (!libUser) return;
    if (!confirm('Remove this note from the library? It stays in your notebook.')) return;
    const result = window.LIBRARY.deleteEntry(id, libUser.userId);
    if (result.ok) {
